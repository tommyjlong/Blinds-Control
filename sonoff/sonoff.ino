...
int16_t requested_servo_pwm[MAX_PWMS];      //TOMMY Check to see if a request to move the servo is outstanding
int16_t last_servo_pwm[MAX_PWMS];           //TOMMY Check for last position setting of servo.
...
//  uint8_t user_append_index = 0;
  ...
  uint16_t index;
  uint32_t address;
  uint8_t is_blinds_command = 0;  //TOMMY
...
*/
      ...
      if ((payload >= 0) && (payload <= Settings.pwm_range) && (pin[GPIO_PWM1 + index -1] < 99)) {
        ...
        if (!Settings.flag3.slow_servo) {
          ...
          requested_servo_pwm[index -1] = (bitRead(pwm_inverted, index -1) ? Settings.pwm_range - payload : payload) ;
          snprintf_P(log_data, sizeof(log_data), "SRV: Requested=%d Last= %d ", requested_servo_pwm[index -1], last_servo_pwm[index-1]);
          AddLog(LOG_LEVEL_DEBUG_MORE);
          is_blinds_command = 1;
        } 
      }
...
//    if ((1 == payload) || ((payload >= 100) && (payload <= 4000))) {
      if ((1 == payload) || ((payload >= 50) && (payload <= 4000))) {  //TOMMY Allow for min of 50Hz
...
//if (mqtt_data[0] != '\0') MqttPublishPrefixTopic_P(RESULT_OR_STAT, type);
  if (mqtt_data[0] != '\0') {
    if (Settings.flag3.slow_servo_retain_results && is_blinds_command ) { //TOMMY if blinds command and slow_servo mode then retain published stat results
      MqttPublishPrefixTopic_P(RESULT_OR_STAT, type, true);
    }
    else{
      MqttPublishPrefixTopic_P(RESULT_OR_STAT, type);
    }
